...
export function SponsorGroups({ sponsors, client = supabase, logoPreview = null }) {
  return (
          ...
          <div key={tier.key} className="flex min-w-0 flex-col items-center">
            ...
            <div className="flex flex-wrap justify-center gap-6 w-full max-w-5xl">
              {companies.map((sponsor) => (
                <SponsorCard key={sponsor.id || 'preview'} sponsor={sponsor} size={tier.size} client={client} logoPreview={logoPreview} />
              ))}
            </div>
          </div>
        );
